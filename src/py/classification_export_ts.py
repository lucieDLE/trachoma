...
def main(args):
    ...
    ttdata = TTDataModule(df_train, df_val, df_test, batch_size=args.batch_size, num_workers=args.num_workers, img_column='image', class_column="patch_class", valid_transform=EvalTransforms(448))
    ...
    trainer = Trainer(
        ...
        callbacks=[early_stop_callback, checkpoint_callback, QuantizationAwareTraining(qconfig='qnnpack', observer_type="histogram")],
        devices=torch.cuda.device_count(), 
        accelerator="gpu", 
        strategy=DDPStrategy(find_unused_parameters=False),
        log_every_n_steps=args.log_every_n_steps
    )
    trainer.validate(model, datamodule=ttdata, ckpt_path=args.model)

    # torch.save(model.model.state_dict(), os.path.join(args.out, "model.pt"))
