...
class Features(tf.keras.layers.Layer):
    def __init__(self):
        ...
        q0 = tf.expand_dims(self.resnet(x[:,0:224,0:224,:], training=training), axis=1)
        q1 = tf.expand_dims(self.resnet(x[:,224:,0:224,:], training=training), axis=1)
        q2 = tf.expand_dims(self.resnet(x[:,0:224,224:,:], training=training), axis=1)
        q3 = tf.expand_dims(self.resnet(x[:,224:,224:,:], training=training), axis=1)

        x = layers.Concatenate(axis=1)([q0, q1, q2, q3])

        x, w_a = Attention(128)(x, x)
...
class TTModel(tf.keras.Model):
    def __init__(self):
        ...
        self.model_prediction = tf.keras.Model(inputs=x0, outputs=[x, w_a])
        self.model_prediction.summary()

    def call(self, x):

        x = self.TD(x)
        x, w_a_f = self.model_prediction(x)
...
checkpoint_path = "/work/jprieto/data/remote/EGower/jprieto/train/stack_training_10082021_16_544_2class/cp-{epoch:04d}.ckpt"
