...
class TTNetFeatures(nn.Module):
    def __init__(self):
        super(TTNetFeatures, self).__init__()
...
class TTNet(nn.Module):
    def __init__(self, feat=None):
        ...
        self.PE = PositionalEncoding(d_model=512, max_len=16)
    ...
    def forward(self, x):
        ...
        x_a, w_a, w_s = self.Attention(x_pe, x_pe)
...
def main(rank, world_size):
    ...
    model_features = TTNetFeatures()
    model_features.load_state_dict(torch.load("train/train_patch_efficientnet_b4_14032022/model.pt"))
    model_features.Features.requires_grad = False

    model = TTNet(model_features.Features)
    ...
    early_stop = EarlyStopping(patience=10, verbose=True,
        path='train/torch_stack_efficient_torch_14032022/model.pt')
