...
for fold in range(0, 5):
    ...
    with open(csv_path.replace(".csv", "_fold" + str(fold) + "_30042022_prediction.pickle"), 'rb') as f:
        dataset_predict = pickle.load(f)

    print(classification_report(test_df["patch_class"], np.argmax(dataset_predict[0], axis=1)))


    test_df["pred"] = np.argmax(dataset_predict[0], axis=1)


    test_dfs.append(test_df)

train_df = pd.concat(test_dfs)

train_df_oksamples = train_df.query('(patch_class == pred) or (patch_class > 0 and pred > 0)')
train_df_oksamples.to_csv("/work/jprieto/data/remote/EGower/hinashah/Analysis_Set_20220326/trachoma_bsl_mtss_besrat_field_patches_train_20220326_train_clean.csv", index=False)

print(bcolors.OKGREEN, "Good samples", bcolors.ENDC)
print(train_df_oksamples.groupby('patch_class').count())




train_df_badsamples = train_df.query('~((patch_class == pred) or (patch_class > 0 and pred > 0))')
train_df_badsamples.to_csv("/work/jprieto/data/remote/EGower/hinashah/Analysis_Set_20220326/trachoma_bsl_mtss_besrat_field_patches_train_20220326_train_bad.csv", index=False)

print(bcolors.FAIL, "Bad samples", bcolors.ENDC)
print(train_df_badsamples.groupby('patch_class').count())
...
print(bcolors.OKBLUE, "Original", bcolors.ENDC)
print(train_df.groupby('patch_class').count())