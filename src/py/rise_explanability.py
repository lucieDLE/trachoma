...
class TTModelScore(tf.keras.Model):
    def __init__(self, features = None):
        super(TTModelScore, self).__init__()
...
model_scores = TTModelScore()
model_scores.features = model.features
model_scores.V = model.V
model_scores.A = model.A
model_scores.build(input_shape=(None, 448, 448, 3))
...
for idx, (X, Y) in tqdm(enumerate(dataset)):
    ...
    out_name = "/work/jprieto/data/remote/EGower/jprieto/masks_rise_explainability/trachoma_bsl_mtss_besrat_field_test_20220326_stacks_" + str(idx) + ".nrrd"
    if not os.path.exists(out_name):

        saliency_maps = []
        for x in X[0]:
            x = x*masks
            x = np.concatenate([model_scores.predict_on_batch(x[i:i+64]) for i in range(0, N, 64)])
            x = x.T.dot(masks.reshape(N, -1)).reshape(448, 448, 1)
            x = x / N / p1
