...
def make_model():
    ...
    x0 = tf.keras.Input(shape=[None, 256, 256, 3])
...
class DatasetGenerator:
    def __init__(self, df, unique_class_weights):
        ...
        self.dataset = tf.data.Dataset.from_generator(self.generator,
            ...
            output_shapes=((None, 256, 256, 3), [1], [1])
            )
    ...
    def generator(self):
        ...
        for idx, row in self.df.iterrows():
            ...
            img_np = tf.image.random_crop(img_np, size=(8, 256, 256, 3))

            img_np = [ndimage.rotate(im, np.random.random() * 180 - 90, reshape=False, mode='reflect') for im in img_np]
...
df = pd.read_csv("/work/jprieto/data/remote/EGower/jprieto/trachoma_normals_healthy_sev123_05182021_stack_32_384_09072021_train.csv")
...
checkpoint_path = "/work/jprieto/data/remote/EGower/train/stack_training_09072021_2class"
